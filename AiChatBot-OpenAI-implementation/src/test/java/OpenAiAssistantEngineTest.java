...
void testCancelRun() {
    ...
    // Cancel any existing in-progress run first
    String runStatus = engine.retrieveRunStatus(testThreadId);
    if (runStatus != null && runStatus.contains("\"status\":\"in_progress\"")) {
        JSONObject statusObj = new JSONObject(runStatus);
        String existingRunId = statusObj.getString("id");
    ...
    // Check if the run is already completed before canceling
    String runInfo = engine.retrieveRun(testThreadId, runId);
    JSONObject runJson = new JSONObject(runInfo);
    String status = runJson.getString("status");

    if (status.equals("completed") || status.equals("failed") || status.equals("cancelled")) {
        System.out.println("Run already ended with status: " + status + " — skipping cancel test.");
        return;
    }
